...
// Find the line with "return (" - keep everything before it
let returnIdx = 0;
for (let i = 0; i < lines.length; i++) {
  if (lines[i].trim() === "return (") { returnIdx = i; break; }
}
console.log("Return found at line:", returnIdx);
const top = lines.slice(0, returnIdx).join("\n");
    ...
    <div className="min-h-screen flex">
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-slate-800 via-slate-900 to-black flex-col justify-between p-12 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-4xl font-black text-white leading-tight mb-4">Super Admin<br /><span className="text-blue-400">Control Center</span></h2>
          <p className="text-gray-400 text-sm leading-relaxed mb-8">Complete control over all companies, users, analytics and platform settings.</p>
          <div className="space-y-3">
            {[{icon:"🏢",label:"Manage all companies"},{icon:"📊",label:"Platform-wide analytics"},{icon:"👥",label:"Cross-company transfers"},{icon:"🔐",label:"Security and compliance"}].map((f,i) => (
